...
const foodDatabase: FoodItem[] = [
  ...
  { id: '11', name: 'Quinoa', calories: 222, carbs: 39, protein: 8, fat: 3.6, glycemicIndex: 53 },
  { id: '12', name: 'Sweet Potato', calories: 112, carbs: 26, protein: 2, fat: 0.1, glycemicIndex: 70 },
];

// Sample user profile (in a real app, this would come from authentication)
const userProfile = {
  id: 'user123',
  name: 'John Doe',
...
const DietPlanner: React.FC = () => {
  ...
  useEffect(() => {
    const savedPlansString = localStorage.getItem(`dietPlans_${userProfile.id}`);
    if (savedPlansString) {
      try {
        setSavedPlans(JSON.parse(savedPlansString));
      } catch (e) {
  ...
  const saveDietPlan = () => {
    ...
    // Add to saved plans
    const updatedSavedPlans = [...savedPlans.filter(plan => plan.id !== planToSave.id), planToSave];
    setSavedPlans(updatedSavedPlans);

    // Save to localStorage
    localStorage.setItem(`dietPlans_${userProfile.id}`, JSON.stringify(updatedSavedPlans));
  ...
  const deleteSavedPlan = (planId: string) => {
    const updatedSavedPlans = savedPlans.filter(plan => plan.id !== planId);
    setSavedPlans(updatedSavedPlans);
    localStorage.setItem(`dietPlans_${userProfile.id}`, JSON.stringify(updatedSavedPlans));
